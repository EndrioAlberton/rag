...
| MySQL, Redis, ChromaDB | auto | Started automatically by Quarkus Dev Services via Docker/Testcontainers |
| Frontend (Vite dev) | 5173 | `cd frontend && npm run dev` (optional — built frontend is served by Quarkus) |

### Key caveats

- **Docker is required.** Quarkus Dev Services uses Testcontainers to start MySQL, Redis, and ChromaDB. The Docker daemon must be running before `./mvnw quarkus:dev`. In the cloud VM, start it with `sudo dockerd &>/tmp/dockerd.log &` and wait a few seconds.
...
- **Startup takes ~60-90s** on first run due to Docker image pulls (MySQL, Redis, ChromaDB, Testcontainers Ryuk) and document ingestion (scraping 19 URLs + local files).
